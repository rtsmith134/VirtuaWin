...
int redrawWindow(HWND hwnd)
{
  ...
  // Get the number of milliseconds that have passed since the system was started
  // if less that 100 milliseconds since the last call return
  ...
  if(dragged)
  {
    ...
    if(((ww=r.right-r.left) > 14) && ((hh=r.bottom - r.top) > 14))
    {
      ...
      if(icn != NULL)
      {
		...
		//Draws an icon or cursor into the specified device context, performing the 
		//specified raster operations, and stretching or compressing the icon or cursor as specified. 
        DrawIconEx(hdc, 
                   ll+WINW*deskx, 
                   tt+WINH*desky, 
                   icn,
